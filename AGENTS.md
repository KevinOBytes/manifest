...
## Artifact Contract
1. `soul/<role>/` is one role persona artifact directory, identified by `SOUL.md`.
2. `.agents/skills/<skill>/` is one portable skill artifact directory, identified by `SKILL.md`.
3. Marker files are plain Markdown, and loaders may return the containing artifact directory path.
